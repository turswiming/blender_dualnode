...
class ImageOperation : public NodeOperation {
 public:
  ...
  void compute_output(StringRef identifier)
  {
    ...
    GPUShader *shader = shader_pool().acquire(get_shader_name(identifier));
  ...
  {
    if (identifier == "Alpha") {
      return "compositor_convert_color_to_alpha";
    }
...
class RenderLayerOperation : public NodeOperation {
 public:
  ...
  void execute() override
  {
    ...
    GPUShader *shader = shader_pool().acquire("compositor_convert_color_to_alpha");
    GPU_shader_bind(shader);

    const int input_unit = GPU_shader_get_texture_binding(shader, "input_sampler");
    GPU_texture_bind(pass_texture, input_unit);

    alpha_result.bind_as_image(shader, "output_image");
