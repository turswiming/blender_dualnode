...
void main()
{
  TEST(eevee_shadow, DirectionalClipmapLevel)
  {
    ...
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 0.0)), light.clipmap_lod_min);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 0.49)), 0);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 0.5)), 0);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 0.51)), 1);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 0.99)), 1);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 1.0)), 1);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 1.01)), 2);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 12.5)), 5);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 12.51)), 5);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 15.9999)), 5);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 16.0)), 5);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 16.00001)), 6);
    EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(fac * 5000.0)), light.clipmap_lod_max);
    /* Produces NaN / Inf, Undefined behavior. */
    // EXPECT_EQ(shadow_directional_clipmap_level(light, vec3(FLT_MAX)), light.clipmap_lod_max);
  }
