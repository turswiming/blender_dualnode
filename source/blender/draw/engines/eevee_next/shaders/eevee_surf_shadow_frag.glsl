...
void write_depth(
    ivec2 texel_co, int lod, ivec2 tile_co, ivec2 texel_in_page, int view_id, float depth)
{
  if (all(equal((texel_co % (1 << lod)), ivec2(0)))) {
...
void main()
{
  ...
  ivec2 texel_in_page = texel_co % pages_infos_buf.page_size;
  float depth = gl_FragCoord.z;

  write_depth(texel_co, 0, tile_co, texel_in_page, shadow_interp.view_id, depth);
  write_depth(texel_co, 1, tile_co, texel_in_page, shadow_interp.view_id, depth);
  write_depth(texel_co, 2, tile_co, texel_in_page, shadow_interp.view_id, depth);
  write_depth(texel_co, 3, tile_co, texel_in_page, shadow_interp.view_id, depth);
  write_depth(texel_co, 4, tile_co, texel_in_page, shadow_interp.view_id, depth);
}
